...
```text
release/Night-Patrol-0.2.1-mac-arm64.dmg
release/Night-Patrol-0.2.1-mac-arm64.zip
```
...
```text
Night-Patrol-0.2.1-mac-arm64.dmg
Night-Patrol-0.2.1-mac-arm64.zip
Night-Patrol-0.2.1-win-x64.exe
Night-Patrol-0.2.1-win-x64.zip
```
...
4. 打 `v0.2.1-demo` tag，让 GitHub Actions 构建 macOS 和 Windows 包。
5. 补正式图标。
6. 截图、录屏、写 Release。
7. 发给第一批朋友试玩。
